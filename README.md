# Zbiór terminalowych różności

## Lista programów
- [gr](/gr.md)
- [curl](/curl.md)
- [speedometer](/speedometer.md)
- [ranger](/ranger.md)
- [lnav](/lnav.md)
- [tmux](/tmux.md)
- [pup](/pup.md)
- [yq/xq](/yq.md)
- [uptimed](/uptimed.md)
- [psysh](/psysh.md)
- [htop](/htop.md)
- [neovim](/neovim.md)
- [fzf](/fzf.md)
- [jq](/jq.md)
- [parallel](/parallel.md)
- [pcregrep](/pcregrep.md)
- [pgcli](/pgcli.md)
- [termdown](/termdown.md)

## Zasoby
### [Bash Hackers Wiki](https://wiki.bash-hackers.org/start)
- [Parameter expansion](https://wiki.bash-hackers.org/syntax/pe)
- [Process substitution](https://wiki.bash-hackers.org/syntax/expansion/proc_subst)
- [Command substitution](https://wiki.bash-hackers.org/syntax/expansion/cmdsubst)
- [Patterns and pattern matching](https://wiki.bash-hackers.org/syntax/pattern)
- [Brace expansion](https://wiki.bash-hackers.org/syntax/expansion/brace)

### Colors and formatting
- [Bash tips: Colors and formatting (ANSI/VT100 Control sequences)](https://misc.flogisoft.com/bash/tip_colors_and_formatting)
- [256 COLORS - CHEAT SHEET](https://jonasjacek.github.io/colors/)
- [True Colour (16 million colours) support in various terminal applications and terminals](https://gist.github.com/XVilka/8346728)
- [tic manual](https://linux.die.net/man/1/tic)
- [infocmp manual](https://linux.die.net/man/1/infocmp)

### git
- [15 Git Commands You May Not Know](https://dev.to/zaiste/15-git-commands-you-may-not-know-4a8j)
- [git diff --color-moved=<mode>](https://git-scm.com/docs/git-diff#Documentation/git-diff.txt---color-movedltmodegt)
- [git grep -P](https://git-scm.com/docs/git-grep#Documentation/git-grep.txt--P) + [grep.patternType](https://git-scm.com/docs/git-grep#Documentation/git-grep.txt-greppatternType)

## Moja konfiguracja
Część programów posiada pliki konfiguracyjne. Jeśli modyfikowałem któryś z nich, to najprawdopodobniej zmodyfikowana przez mnie wersja znajduje się w [moim repozytorium dotfiles](https://github.com/hxv/dotfiles).
